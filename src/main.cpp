...
namespace Transform {
	mvp_matrix_t mvp_matrix1;
	mvp_matrix_buffer_t mvp_matrix_buffer1;
	mvp_matrix_t point_light_mvp1;
	mvp_matrix_buffer_t point_light_mvp_buffer1;
}
...
namespace ConstantValue {
	constant_param_t constant_param;
	constant_param_buffer_t constant_param_buffer;
}
...
int load_lights(const device_t* device, const swapchain_t* swapchain) {
	PointLight::create_light_attri_buffer(&PointLight::point_light_attri_buffer1, device, swapchain);
	return 0;
}

int load_constant(const device_t* device, const swapchain_t* swapchain) {
	ConstantValue::create_constant_param_buffer(&ConstantValue::constant_param_buffer, device, swapchain);
	return 0;
}

int update_constant(first_person_camera_t* camera, swapchain_t* swapchain) {
	ConstantValue::constant_set_param param;
	param.camera = camera;
	ConstantValue::update_constant_param(&ConstantValue::constant_param, &ConstantValue::constant_param_buffer, &param, swapchain);
	return 0;
}
...
int update_transform(first_person_camera_t* camera, swapchain_t* swapchain) {
	// memcpy((char*)cube_transform->data + sizeof(ubo), &ubo, sizeof(ubo));
	Transform::mvp_set_param param;
	param.angle = ConstantValue::angle;
	param.camera = camera;
...
int init_camera(first_person_camera_t* camera) {
	...
	camera->lastX = ConstantValue::width / 2;
	camera->lastY = ConstantValue::height / 2;
...
int create_shading_pass(shading_pass_s* pass, const device_t* device, const swapchain_t* swapchain, const render_pass_t* render_pass) {
	...
	VkWriteDescriptorSet descriptor_set_writes[] = {
		...
		{.dstBinding = 2, .pBufferInfo = &descriptor_buffer_infos[2]},
	};
...
int record_render_frame_commands(VkCommandBuffer cmd, application_t* app, uint32_t swapchain_index, ImDrawData* draw_data) {
	...
	VkClearValue clear_values[] = {
		{.color = {.float32 = {ConstantValue::clearColor.x,ConstantValue::clearColor.y, ConstantValue::clearColor.z, ConstantValue::clearColor.w} } },
	...
	vkCmdBindVertexBuffers(cmd, 0, 1, &Mesh::cube1_mesh_buffer.vertices.buffers->buffer, offsets);
...
int update_application(application_t* app) {
	...
	glfwSetWindowSize(app->swapchain.window, (int)ConstantValue::width, (int)ConstantValue::height);
	application_updates_t updates = { VK_FALSE };
	load_scene( &app->device);
	init_camera(&app->camera);
	load_transform( &app->device, &app->swapchain);
	load_constant(&app->device, &app->swapchain);
	load_lights( &app->device, &app->swapchain);
	// update_transform(&app->cube_transform, &app->camera, &app->swapchain);
...
int startup_application(application_t* app) {
	...
	if (create_or_resize_swapchain(&app->swapchain, &app->device, VK_FALSE, application_display_name, ConstantValue::width, ConstantValue::height, VK_TRUE)) {
...
int handle_frame_input(application_t* app) {
	if (glfwGetKey(app->swapchain.window, GLFW_KEY_ESCAPE)== GLFW_PRESS) {
		printf("Escape pressed. Shutting down.\n");
		return 1;
	}
	record_frame_time();
	// Define the user interface for the current frame
	control_camera(&app->camera, app->swapchain.window, get_frame_time());
	
	update_transform(&app->camera, &app->swapchain);
	update_constant( &app->camera, &app->swapchain);
	update_lights(&app->swapchain);
...
int main() {
	...
	while (!glfwWindowShouldClose(app.swapchain.window)) {
		...
		while (cost_time < frame_time) {
			end = system_clock::now();
			duration = duration_cast<microseconds>(end - start);
			cost_time = double(duration.count()) * microseconds::period::num / microseconds::period::den;
			// cout << cost_time << " " << frame_time << endl;
		}
	}
