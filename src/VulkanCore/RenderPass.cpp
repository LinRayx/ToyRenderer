...
namespace Graphics {
	...
	void RenderPass::CreateCSMRenderPass(VkImage depthImg, VkImageView depthView, vector<VkImageView>& cascadeViews, VkFormat format, int width, int height)
	{
		...
		if (vkCreateRenderPass(Vulkan::getInstance()->device.device, &renderPassCreateInfo, nullptr, &renderPass) != VK_SUCCESS) {
			throw std::runtime_error("failed to create render pass!");
		}

		framebuffers.resize(cascadeViews.size());

		for (uint32_t i = 0; i < cascadeViews.size(); i++) {
